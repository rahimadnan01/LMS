...
const showPlaylists = wrapAsync(async (req, res) => {
  const allPlaylists = await Playlist.aggregate([
    ...
    {
      $match: { "audios.0": { $exists: true } },
    },
  ]);
  if (!allPlaylists) {
    throw new ApiError(404, "Playlist may not found");
  }
  res
    .status(200)
    .json(new ApiResponse(200, "Playlist shown successfully", allPlaylists));
});
const getPlaylist = wrapAsync(async (req, res) => {
  let { playlistId } = req.params;
  let objectId = new mongoose.Types.ObjectId(playlistId);
